...
每个模块都是自包含的，所有 js 文件都通过 Browserify 进行导出、混淆压缩合并。  
...
4) 打开模块父 ```index.js``` 文件 (譬如 ```modules/index.js``` )，为新模块新增依赖 requirement。切记是 require 整个模块目录 ( browserify 会寻找 index.js 文件作为其他模块依赖的入口)。
